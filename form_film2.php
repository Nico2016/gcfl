...
////////////////////////////on iclut nos packages composer//////////////////////////////////////////
...
if (!empty($_POST)) {
	...
	if ($fil_id > 0) {
		...
		$insertInto = '
			INSERT INTO film (fil_titre, fil_annee, fil_synopsis, fil_description, fil_acteurs, fil_filename, fil_affiche,cat_id,sup_id)
...
// Récupère tous les films pour générer le menu déroulant des films
$sql = '
	SELECT fil_id, fil_titre
	FROM film
';
$pdoStatement = $pdo->query($sql);
if ($pdoStatement && $pdoStatement->rowCount() > 0) {
...
<body>
	<form action="" method="get">
		<fieldset>
				...
				<tr>
					<td>Titre IMDb :&nbsp;</td>
					<td><input type="text" name="imdb_titre" value="<?php echo $imdb_titre; ?>"/></td>
	...
	<form action="" method="get">
			<select name="id">
				<option value="0">ajouter un film</option>
				<!-- je parcours les catégories pour remplir le menu déroulant des catégories -->
				<?php foreach ($filmList as $curFilm) : ?>
				<option value="<?php echo $curFilm['fil_id']; ?>"<?php echo $currentId == $curFilm['fil_id'] ? ' selected="selected"' : ''; ?>><?php echo $curFilm['fil_titre']; ?></option>
				<?php endforeach; ?>
			</select>
		...
		<fieldset>
			...
			<tr>
				...
				<td><select name="fil_annee">
					<option value="">choisissez :</option>
					<?php for ($annee=date('Y');$annee>1900;$annee--) : ?>
